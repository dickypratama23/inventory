	...
	<div class="ui segment">
		...
		<form class="ui form" action="{{ route('pp.store') }}" method="post">
			...
			<div class="field">
				...
				<input type="text" name="no_pp" readonly value="{{ $NO_PP }}">
			</div>
			...
			<div class="field">
				...
				<select class="ui search dropdown" name="jenis_pp">
					...
					<option value="cabang" @if($J_PP->jenis_pp == "cabang") selected @else disabled @endif >Cabang</option>
					<option value="ho" @if($J_PP->jenis_pp == "ho") selected @else disabled @endif >HO</option>
				</select>
			</div>
            ...
            <tr>
                ...
                <td>
                    <a href="{{ route('pp.delete_barang', ['id' => $detail->nomor_pp, 'id2' => $detail->barang_id]) }}" class="fluid ui red button">Hapus</a>
                </td>
            </tr>
            @endforeach
				</tbody>
		...
		<br>
		<form action="{{ route('pp.selesai', ['id' => $NO_PP]) }}" method="post">
				{{ csrf_field() }}
